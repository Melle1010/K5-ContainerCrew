...
The AI‑driven functionality is built around Gemini, which is used to generate content based on user input. The frontend sends a prompt to the AI Content API, which validates the request and forwards it to the LLM Proxy. The LLM Proxy then communicates directly with Gemini by using an API key. When it receives the generated text it returns it to the AI Content API which in turn sends the response back to the user in the frontend. This architecture ensures separation of concerns while also making it easy to observe how data flows through the system. The AI feature is fully functional both locally and in production.
...
1. Open your terminal and navigate to the root repository folder (the same level as `docker-compose.yml`).
2. Create a `.env` file in this directory. This file is used by Docker to inject all required environment variables into the containers at runtime.
3. Use the following environment template and replace the placeholder with your actual key:

```text
ServiceB__ApiKey=super-secret-key-123
LlmProxy__BaseUrl=http://llmproxy-app:8080/
GEMINI_API_KEY=YOUR_GEMINI_API_KEY

```

4. Run the following command to build the images and launch the containers:
...
Once the containers are running and healthy, the applications are available at:

* Frontend - http://localhost:8080/
* Backend (Scalar UI) - http://localhost:5292/scalar/
